...
Throughout the course of working on this project, we have learned many lessons and there are many takeaways. This project let us work though almost the entire data lifecycle. From finding datasets, completing profiling and quality analysis on the data, merging the datasets, and more we were able to transform existing data into our own. We were able to implement so many new techniques and softwares that before this course we had never heard of or used in practice. We used new things like the Chicago data portal. We exported datasets and chose what type of format we wanted to work with it in. We also used OpenRefine to clean the data and standardize the values. This method was new to us and we found it much easier than using python to do this. This was a new skill we got to implement because of the course. We also learned how to assess Data Quality and had the opportunity to use those new skills as well. Data Integration was a topic we had heard about in the past, but learned new ways to do this. We also used Snakemake and made a workflow. This project made us think about data in a different way and showed us different ways to analyze data. 

Future work that could be done in regards with this project and research question is going beyond the 2023 data to other years. There is energy benchmark dating back to ~2019 and environmental complaint data from just as long. We would be interested in seeing if trends stay consistent from year to year, if the energy scores change drastically throughout the years, and what types of complaints are associated with different neighborhoods and property types. There is so much more that can be done with our merged dataset and many new questions that emerged from the analysis. It also would have been interesting if we could have looked at the number of properties in neighborhoods. We would have to merge the data differently to do this because we are interested in all buildings, not just buildings with nearby complaints. 

Other future work we will do will consist of using all of the new methods and lessons we have learned about data science in the future. We have gained so much additional knowledge and experience as a result of this course and got to apply these lessons through this project. For future courses and careers, we are well prepared to develop projects the right way because of this experience. We understand why different things are important to include in projectsand how to take data from concepts to results. 

The most important lessons that we will take forward from this course that we had the opportunity to apply through this project are the importance of reproducability and transparency. Data science used to just be about analyzing csvs in previous courses. Now it is so much more than that. From metadata, version histories, and quality assessments, our understanding of data science has grown drastically. All of our future work will be completed by maintaining ethics and data conformance regulations because of the modules of this course.
